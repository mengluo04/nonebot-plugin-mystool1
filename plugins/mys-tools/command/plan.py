...
from nonebot.adapters.onebot.v11 import MessageSegment as OneBotV11MessageSegment, Adapter as OneBotV11Adapter, \
    MessageEvent as OneBotV11MessageEvent, GroupMessageEvent as OneBotV11GroupMessageEvent, Bot
...
async def _(bot: Bot, event: Union[GeneralMessageEvent], matcher: Matcher, command_arg=CommandArg()):
    ...
    if command_arg:
        ...
        await manually_game_sign.send("⏳开始游戏签到...",at_sender=True)
...
async def _(event: Union[GeneralMessageEvent], matcher: Matcher, command_arg=CommandArg()):
    ...
    await manually_bbs_sign.send("⏳开始执行米游币任务...",at_sender=True)
...
CommandRegistry.set_usage(
    ...
    CommandUsage(
        name="原神便签",
        description="手动查看原神实时便签，即原神树脂、洞天财瓮等信息"
    )
)
...
manually_starrail_note_check = on_command(
    plugin_config.preference.command_start + '星穹铁道便签',
    aliases={
        plugin_config.preference.command_start + '铁道便签',
        plugin_config.preference.command_start + '星铁便签',
    },
    priority=5,
    block=True
)
...
CommandRegistry.set_usage(
    ...
    CommandUsage(
        name="星穹铁道便签",
        description="手动查看星穹铁道实时便签，即开拓力、每日实训、每周模拟宇宙积分等信息"
    )
)
...
manually_zzz_note_check = on_command(
    plugin_config.preference.command_start + '绝区零便签',
    aliases={
        plugin_config.preference.command_start + 'zzz便签',
    },
    priority=5,
    block=True
)
CommandRegistry.set_usage(
    manually_zzz_note_check,
    CommandUsage(
        name="绝区零便签",
        description="手动查看绝区零实时便签，即电量、每日实训、每周模拟宇宙积分等信息"
    )
)


@manually_zzz_note_check.handle()
async def _(event: Union[GeneralMessageEvent], matcher: Matcher):
    """
    手动查看星穹铁道便签（sr）
    """
    ...
    if not user or not user.accounts:
        await manually_game_sign.finish(f"⚠️你尚未绑定米游社账户，请先使用『{COMMAND_BEGIN}登录』进行登录")
    await zzz_note_check(user=user, user_ids=[user_id], matcher=matcher)
...
async def perform_game_sign(
    ...
    for account in user.accounts.values():
        ...
        if game_record_status.login_expired:
            if matcher:
                await matcher.send(f"⚠️账户 {account.display_name} 登录过期，请重新登录", at_sender=True)
            else:
                for user_id in user_ids:
                    await send_private_msg(
                        user_id=user_id,
                        message=f"⚠️账户 {account.display_name} 登录过期，请重新登录"
                    )
        elif not game_record_status:
            if matcher:
        ...
        for class_type in BaseGameSign.available_game_signs:
            ...
            if (get_info_status and not info.is_sign) or not get_info_status:
                sign_status, mmt_data = await signer.sign(platform=account.platform)
                if sign_status.need_verify:
                    if plugin_config.preference.geetest_url or user.geetest_url:
                        if matcher:
                            await matcher.send("⏳正在尝试完成人机验证，请稍后...", at_sender=True)
                        geetest_result = await get_validate(user, mmt_data.gt, mmt_data.challenge)
                        sign_status, _ = await signer.sign(platform=account.platform, mmt_data=mmt_data,
                ...
                if matcher:
                    try:
                        if isinstance(event, OneBotV11MessageEvent):
                            if isinstance(event, OneBotV11GroupMessageEvent):
                                msg_list.append(msg + onebot_img_msg)
                            else:
                                await matcher.send(msg + onebot_img_msg, at_sender=True)
            ...
            await asyncio.sleep(plugin_config.preference.sleep_time)
        if msg_list:  # 在群聊触发游戏签到将使用合并消息
            def build_forward_msg(msg):
                return {"type": "node", "data": {"nickname": "流萤", "user_id": "100723375", "content": msg}}

            # 受限于LLOnebot，合并转发消息只能使用bot的身份无法自定义
            messages = [build_forward_msg(msg) for msg in msg_list]
    ...
    for account in user.accounts.values():
        ...
        if not finished:
            if not account.mission_games:
                await matcher.send(
                    f'⚠️🆔账户 {account.display_name} 未设置米游币任务目标分区，将跳过执行', at_sender=True)
        ...
        if user.enable_notice or matcher:
            ...
            if not missions_state_status:
                ...
                if matcher:
                    await matcher.send(
                        f'⚠️账户 {account.display_name} 获取任务完成情况请求失败，你可以手动前往App查看', at_sender=True)
                else:
                    for user_id in user_ids:
                        await send_private_msg(
                            user_id=user_id,
    ...
    for account in user.accounts.values():
        ...
        if account.enable_resin or matcher:
            ...
            if not genshin_board_status:
                if matcher:
                    if genshin_board_status.login_expired:
                        ...
                        await matcher.send(f'⚠️账户 {account.display_name} 获取实时便签时被人机验证阻拦',
                                           at_sender=True)
                    await matcher.send(f'⚠️账户 {account.display_name} 获取实时便签请求失败，你可以手动前往App查看',
                                       at_sender=True)
                continue
            ...
            if not matcher:
                ...
                if not do_notice:
                    logger.info(f"原神实时便签：账户 {account.display_name} 树脂:{note.current_resin},未满足推送条件")
                    return
    ...
    for account in user.accounts.values():
        ...
        if account.enable_resin or matcher:
            ...
            if not matcher:
                ...
                if not do_notice:
                    logger.info(
                        f"崩铁实时便签：账户 {account.display_name} 开拓力:{note.current_stamina},未满足推送条件")
                    return
...
async def zzz_note_check(user: UserData, user_ids: Iterable[str], matcher: Matcher = None):
    """
    查看绝区零实时便签函数，并发送给用户任务执行消息。
    ...
    for account in user.accounts.values():
        note_notice_status.setdefault(account.bbs_uid, NoteNoticeStatus())
        zzz_notice = note_notice_status[account.bbs_uid].zzz
        if account.enable_resin or matcher:
            zzz_board_status, note = await zzz_note(account)
            if not zzz_board_status:
                if matcher:
                    if zzz_board_status.login_expired:
                        await matcher.send(f'⚠️账户 {account.display_name} 登录失效，请重新登录', at_sender=True)
                    elif zzz_board_status.no_zzz_account:
                        await matcher.send(f'⚠️账户 {account.display_name} 没有绑定任何星铁账户，请绑定后再重试',
                                           at_sender=True)
                    elif zzz_board_status.need_verify:
                        await matcher.send(f'⚠️账户 {account.display_name} 获取实时便签时被人机验证阻拦',
                                           at_sender=True)
                    await matcher.send(f'⚠️账户 {account.display_name} 获取实时便签请求失败，你可以手动前往App查看',
                                       at_sender=True)
            ...
            if not matcher:
                ...
                """记录是否需要提醒"""
                # 体力溢出提醒
                if note.current_energy >= account.user_energy_threshold:
                    # 防止重复提醒
                    if not zzz_notice.current_energy_full:
                        if note.current_energy >= note.max_energy:
                            note_notice_status.curr = True
                            msg += '❕您的电量已经溢出\n'
                            if note.current_vitality != note.max_vitality:
                                msg += '❕您的每日活跃度未完成\n'
                            do_notice = True
                        elif not zzz_notice.current_energy_full:
                            zzz_notice.current_energy_full = False
                            zzz_notice.current_energy = True
                            msg += '❕您的电量已达到提醒阈值\n'
                            if note.current_vitality != note.max_vitality:
                                msg += '❕您的每日活跃度未完成\n'
                            do_notice = True
                else:
                    zzz_notice.current_energy = False
                    zzz_notice.current_energy_full = False

                if not do_notice:
                    logger.info(
                        f"绝区零实时便签：账户 {account.display_name} 电量:{note.current_energy},未满足推送条件")
                    return

            if note:
                # 经营状态
                if note.sale_state == 'SaleStateDone':
                    sale_state = '待结算'
                elif note.sale_state == 'SaleStateDoing':
                    sale_state = '经营中'
                elif note.sale_state == 'SaleStateNo':
                    sale_state = '未开始'
                else:
                    sale_state = '获取状态失败'
                # 刮刮卡
                if note.card_sign == 'CardSignNo':
                    card_sign = '未刮'
                elif note.card_sign == 'CardSignDone':
                    card_sign = '已刮'
                else:
                    card_sign = '获取状态失败'
                msg += "❖绝区零·实时便签❖" \
                       f"\n⏳电量：{note.current_energy} / {note.max_energy}" \
                       f"\n⏱剩余恢复时间：{note.restore_recover_text}" \
                       f"\n📒每日活跃度：{note.current_vitality} / {note.max_vitality}" \
                       f"\n🌌刮刮卡：{card_sign}" \
                       f"\n🌌经营状态：{sale_state}"
            else:
                msg += '服务器错误'
...
async def auto_note_check():
    ...
    自动查看实时便签
    """
    logger.info(f"{plugin_config.preference.log_head}开始执行自动便签检查")
    for user_id, user in get_unique_users():
        user_ids = [user_id] + list(get_all_bind(user_id))
        await genshin_note_check(user=user, user_ids=user_ids)
        await starrail_note_check(user=user, user_ids=user_ids)
        await zzz_note_check(user=user, user_ids=user_ids)
    logger.info(f"{plugin_config.preference.log_head}自动便签检查执行完成")
