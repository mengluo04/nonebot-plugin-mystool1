...
from ..model import GameRecord, GameInfo, BaseApiStatus, MmtData, GeetestResult, \
    GetCookieStatus, \
    CreateMobileCaptchaStatus, GeetestResultV4, GenshinNote, GenshinNoteStatus, \
    GetFpStatus, StarRailNoteStatus, StarRailNote, ZzzNote, ZzzNoteStatus, UserAccount, BBSCookies, \
    plugin_env, plugin_config, QueryGameTokenQrCodeStatus
...
URL_ZZZ_NOTE_BBS = "https://api-takumi-record.mihoyo.com/event/game_record_zzz/api/zzz/note"
URL_ZZZ_NOTE_WIDGET = "https://api-takumi-record.mihoyo.com/event/game_record_zzz/api/zzz/widget"
...
class ApiResultHandler(BaseModel):
    ...
    def success(self):
        ...
        是否成功
        """
        return self.retcode == 0 or self.message in ["成功", "OK"]
...
async def zzz_note(account: UserAccount) -> Tuple[
    Union[BaseApiStatus, ZzzNoteStatus],
    Optional[ZzzNote]
]:
    """
    获取绝区零实时便笺
    ...
    game_record_status, records = await get_game_record(account)
    if not game_record_status:
        return ZzzNoteStatus(game_record_failed=True), None
    game_list_status, game_list = await get_game_list()
    if not game_list_status:
        return ZzzNoteStatus(game_list_failed=True), None
    game_filter = filter(lambda x: x.en_name == 'zzz', game_list)
    game_info = next(game_filter, None)
    if not game_info:
        return ZzzNoteStatus(no_starrail_account=True), None
    else:
        game_id = game_info.id
    flag = True
    for record in records:
            ...
            try:
                ...
                async for attempt in get_async_retry(False):
                    with attempt:
                        ...
                        if api_result.login_expired:
                            logger.info(
                                f"绝区零实时便笺: 用户 {account.display_name} 登录失效")
                            logger.debug(f"网络请求返回: {res.text}")
                            return ZzzNoteStatus(login_expired=True), None

                        if api_result.invalid_ds:
                            logger.info(
                                f"绝区零实时便笺: 用户 {account.display_name} DS 校验失败")
                            logger.debug(f"网络请求返回: {res.text}")
                        if api_result.retcode == 1034:
                            logger.info(
                                f"绝区零实时便笺: 用户 {account.display_name} 可能被验证码阻拦")
                            logger.debug(f"网络请求返回: {res.text}")
                        data = api_result.data
                        print(data)
                        obj = {
                            'current_energy': data['energy']['progress']['current'],
                            'max_energy': data['energy']['progress']['max'],
                            'restore': data['energy']['restore'],
                            'current_vitality': data['vitality']['current'],
                            'max_vitality': data['vitality']['max'],
                            'sale_state': data['vhs_sale']['sale_state'],
                            'card_sign': data['card_sign'],
                            'has_signed': data['has_signed']
                        }
                        return ZzzNoteStatus(success=True), ZzzNote.parse_obj(obj)
            except tenacity.RetryError as e:
                if is_incorrect_return(e):
                    logger.exception("绝区零实时便笺: 服务器没有正确返回")
                    logger.debug(f"网络请求返回: {res.text}")
                    return ZzzNoteStatus(incorrect_return=True), None
                else:
                    logger.exception("绝区零实时便笺: 请求失败")
                    return ZzzNoteStatus(network_error=True), None
    if flag:
        return ZzzNoteStatus(no_starrail_account=True), None
